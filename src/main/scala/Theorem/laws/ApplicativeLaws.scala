...
object ApplicativeLaws {
  def allApplicativeLaws[F[_]: Applicative, A, B](ls: F[A], v: A, fn: A => B): Boolean = {
    identityLaw(Applicative[F].pure(v)) && homomorphismLaw(v, fn) && interchangeLaw(v, fn)
  }

  def identityLaw[T[_]: Applicative, A](v: T[A]): Boolean = v.ap(v.pure(identity[A](_))) == v
  def homomorphismLaw[T[_]: Applicative, A, B](v: A, fn: A => B): Boolean = {
    val value: T[A] = Applicative[T].pure(v)

    value.ap(Applicative[T].pure(fn)) == Applicative[T].pure(fn(v))
  }

  def interchangeLaw[T[_]: Applicative, A, B](v: A, fn: A => B): Boolean = {
    val value: T[A] = Applicative[T].pure(v)
