...
object MonadLaws {
  def allMonadLaws[F[_]: Monad, A, B, C](value: A, fn1: A => F[B], fn2: B => F[C]) = {
    val pureValue = Monad[F].pure(value)
    leftIdentity(value, fn1) && rightIdentity(pureValue) && associativity(pureValue, fn1, fn2)
  }

  def leftIdentity[F[_]: Monad, A, B](value: A, fn: A => F[B]) = {
    val cValue = Monad[F].pure(value)

    cValue.flatMap(fn) == fn(value)
  }

  def rightIdentity[F[_]: Monad, A](value: F[A]) = {
    value.flatMap(Monad[F].pure(_)) == value
  }

  def associativity[F[_]: Monad, A, B, C](value: F[A], f1: A => F[B], f2: B => F[C]) = {
